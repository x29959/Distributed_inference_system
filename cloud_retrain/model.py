...
class CNN_LSTM(nn.Module):
	def __init__(self, input_size,num_classes ,output_size, units):
		...
		self.lstm = nn.LSTM(
			...
			num_layers=12,
			dropout=0.5,
			batch_first=True, bidirectional=False)  # if True = (batch, seq, features)
...
class Transformer(nn.Module):
	def __init__(self, input_size, output_size, units, heads):
		...
		self.relu = nn.ReLU()
		self.transformer_block = nn.TransformerEncoderLayer(d_model=output_size, nhead=heads)
		self.linear_1 = nn.Linear(
			in_features=output_size,
			out_features=output_size  # binary classification
		)
		self.softmax = nn.Softmax(dim=1)
...
class Transformer_rev(nn.Module):
	def __init__(self, input_size, output_size, units, heads):
		super(Transformer_rev, self).__init__()
		self.conv1d = nn.Conv1d(
			in_channels=input_size,
			out_channels=output_size,
			kernel_size=3,
		...
		self.transformer_encoder_layer = nn.TransformerEncoderLayer(d_model=output_size, nhead=heads)
		self.transformer_decoder_layer = nn.TransformerDecoderLayer(d_model=output_size, nhead=heads)
...
class Transformer_rev_Complex(nn.Module):
	def __init__(self, input_size, num_classes, units, heads):
		super(Transformer_rev_Complex, self).__init__()
		self.conv1d_1 = nn.Conv1d(in_channels=input_size, out_channels=units, kernel_size=3, padding=1)
		self.conv1d_2 = nn.Conv1d(in_channels=units, out_channels=units, kernel_size=3, padding=1)
		self.relu = nn.ReLU()
		self.dropout = nn.Dropout(p=0.3)
		self.positional_encoding = PositionalEncoding(units)
		encoder_layer = nn.TransformerEncoderLayer(d_model=units, nhead=heads, dim_feedforward=units * 1,batch_first=True)
...
class CRNN(nn.Module):
	def __init__(self, input_size, output_size, units):
		...
		self.rnn = nn.RNN(
			...
			dropout=0.5,
			batch_first=True,
			nonlinearity='relu'  # RNN 預設激活函數是 tanh
		)
