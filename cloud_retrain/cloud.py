...
# Add this before any CUDA operations
def configure_cuda():
    """Configure CUDA environment to avoid cudnn exceptions"""
    os.environ['CUDA_LAUNCH_BLOCKING'] = '1'
    os.environ['TORCH_CUDNN_V8_API_ENABLED'] = '1'
    if torch.cuda.is_available():
        # Force PyTorch to use older CUDNN algorithms that are more stable
...
def load_model(model_name, num_classes):
    if model_name == "Transformer":
        return Transformer_rev_Complex(99, num_classes, 32, 2).to(device)
    elif model_name == "CNN_LSTM":
        return CNN_LSTM(99, num_classes, 64, 32).to(device)
    elif model_name == "CBiLSTM":
        return CBiLSTM(99, 64, 32).to(device)
    elif model_name == "GRUClassifier":
        return GRUClassifier(99, 64, 32, num_classes).to(device)
    elif model_name == "CRNN":
        return CRNN(99, 64, 32).to(device)
...
def train_model(username, model_name_selected, num_classes):
    ...
    try:
        if neural_network == "Transformer":
            model = Transformer_rev_Complex(99, num_classes, 32, 2).to(device)
        elif neural_network == "CNN_LSTM":
            model = CNN_LSTM(99, num_classes, 64, 32).to(device)
        elif neural_network == "CBiLSTM":
            model = CBiLSTM(99, 64, 32).to(device)
        elif neural_network == "GRUClassifier":
            model = GRUClassifier(99, 64, 32, num_classes).to(device)
    ...
    if model_name_selected and model_name_selected != 'train_scratch':
        # Update this path construction to include the neural network type
        model_path = f"./model/{username}/{neural_network}/{model_name_selected}"
        if os.path.exists(model_path):
            ...
            app.logger.warning(f'Selected model {model_name_selected} not found at {model_path}. Training from scratch.')
    ...
    try:
        ...
        app.logger.error(f'Error creating DataLoader: {e}')
        return
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with tqdm(train_loader, unit="batch", disable=False) as tepoch:
            for batch_idx, (x_train, y_train) in enumerate(tepoch):
                if stop_training_event.is_set():
                    app.logger.info(f"Training stopped by user at batch {batch_idx}")
                    break
